...
- **CLAUDE.md vs Auto Memory**: CLAUDE.md=내가 쓰는 지시(전체 로드, 길이 무관하나 짧을수록 준수↑). Auto Memory=Claude가 스스로 쓰는 학습(`~/.claude/projects/<project>/memory/`, **MEMORY.md 첫 200줄/25KB만 매 세션 로드**). → **사용자 104개 메모리 폴더가 바로 이 Auto Memory.** `type:` 스키마 밖 파일은 Claude 자동 생성분일 수 있음.
- 권장 **200줄 이하** (넘으면 준수율↓). 길면 `.claude/rules/` paths로 분리 (import `@path`는 조직화만, 컨텍스트는 그대로 로드됨).
- 로드 순서: managed → user(`~/.claude/CLAUDE.md`) → project(`./CLAUDE.md` 또는 `./.claude/CLAUDE.md`) → local(`./CLAUDE.local.md`, gitignore). 디렉토리 위→아래 concat.
- `claudeMdExcludes`(settings) — 모노레포에서 무관한 CLAUDE.md skip. managed `claudeMd` 키로 조직 전역 주입(개인이 못 끔).
- `@AGENTS.md` import 또는 symlink로 타 에이전트 설정 공유. `/init`은 기존 AGENTS.md·.cursorrules도 흡수.
- 블록 HTML 주석 `<!-- -->`은 컨텍스트 주입 전 제거됨(유지보수 메모용, 토큰 절약).
- `/compact` 후 project-root CLAUDE.md는 재주입됨. 하위 디렉토리 CLAUDE.md는 그 폴더 파일 읽을 때 재로드.
- 안 지켜지면: 시스템 프롬프트가 아니라 user 메시지로 들어감 → 강제 아님. 반드시 실행돼야 하면 hook으로.
...
**설정 파일 (영구, 2026-04 공식 반영)**:
```json
{ "autoMemoryEnabled": false }
```

`/memory` 세션 내에서 토글 가능 (파일 선택기 + Auto Memory 활성/비활성 토글).
...
`settings.json`에서 auto-memory 저장 위치를 변경 가능. `~/` 확장 지원.

**⚠️ 범위 제한** (2026-04 공식 명시):
- **Project settings에서는 허용 안 됨** (`.claude/settings.json`).
- 이유: 공유 레포가 민감 경로(`~/.ssh`, `~/.aws`)로 auto-memory를 redirect하는 공격 방지.
- 허용 위치: Policy, local (`.claude/settings.local.json`), user (`~/.claude/settings.json`).

...
- **스킬 ↔ 서브에이전트 양방향**: ① 스킬에 `context: fork` + `agent` → 스킬 본문이 작업, 에이전트가 실행환경. ② 서브에이전트에 `skills` 필드 → 시작 시 해당 스킬 본문을 preload(에이전트 마크다운이 시스템 프롬프트).
- 내장 에이전트 `Explore`·`Plan`은 시작 시 CLAUDE.md·git status를 안 읽어 컨텍스트 작게 유지.
- 서브에이전트도 자체 Auto Memory 보유 가능 (persistent memory).
- 병렬 다중 세션은 background agents, 세션 간 통신은 agent teams(실험적) 별도 기능.

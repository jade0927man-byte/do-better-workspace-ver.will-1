...
- 플러그인 루트에 추가 가능: `skills/`·`agents/`·`commands/`·`hooks/hooks.json`·`.mcp.json`·**`.lsp.json`(LSP 서버)**·**`monitors/monitors.json`(백그라운드 모니터)**·`bin/`·`settings.json`. (⚠️ `.claude-plugin/`엔 `plugin.json`만)
- 플러그인 `settings.json`은 현재 `agent`·`subagentStatusLine` 키만 지원 (활성 시 메인 스레드를 특정 에이전트로).
- 테스트: `--plugin-dir ./plugin`(.zip도 가능), `--plugin-url <zip URL>`, `/reload-plugins`로 무재시작 갱신.
- 마켓플레이스 2종: `claude-plugins-official`(큐레이션, 기본 제공) / `claude-community`(`/plugin marketplace add anthropics/claude-plugins-community`). 제출 전 `claude plugin validate`.
- standalone(.claude/) vs 플러그인 선택 기준: 공유·배포·버전관리 필요하면 플러그인(이름 `/plugin:skill` 네임스페이스), 개인·실험은 standalone(`/skill`).

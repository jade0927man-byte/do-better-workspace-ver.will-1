...
추가 frontmatter 필드:
- `disable-model-invocation: true` — 다온 자동 호출 차단, `/이름` 수동만. **이름표가 상시 컨텍스트에서 빠짐**(서브에이전트 preload도 차단). 부작용 있는 워크플로우(deploy·send 류)에 적합. ⚠️ 발화 자동 트리거 의존 스킬에 달면 트리거 끊김.
- `user-invocable: false` — `/` 메뉴에서 숨김(다온만 호출). 배경지식용.
- `context: fork` + `agent: <type>` — 스킬을 서브에이전트에서 격리 실행 (SKILL.md 본문이 그 프롬프트가 됨).
- `model` / `effort` / `shell` / `hooks` — 스킬 활성 중 모델·노력수준·셸·라이프사이클 훅 지정.

운영:
- **커스텀 커맨드 = 스킬로 통합**. `.claude/commands/*.md`는 계속 작동, 같은 이름이면 스킬 우선.
- **스킬 컨텍스트 라이프사이클**: 한 번 호출되면 세션 내내 컨텍스트에 머묾(매 턴 토큰 비용) → 본문 짧게. compaction 시 최근 호출분 위주 25K 예산 내 재첨부.
- **이름표 예산 자동 관리**: 예산(컨텍스트 1%) 넘치면 **안 쓰는 스킬 description부터 자동 drop**. `/doctor`로 확인. `skillListingBudgetFraction`·`maxSkillDescriptionChars`(기본 1536)로 조정.
- `skillOverrides`(settings): 스킬별 가시성 4단계 `on`/`name-only`/`user-invocable-only`/`off`. SKILL.md 안 고치고 제어. `/skills` 메뉴 Space 토글.
- 동적 컨텍스트 주입: `` !`명령` `` (스킬 전송 전 셸 출력 삽입), `${CLAUDE_SKILL_DIR}`·`${CLAUDE_SESSION_ID}` 치환.
- 번들 스킬: `/run` `/verify` `/loop` `/code-review` `/batch` `/debug` 등 기본 제공.
- 권한: `Skill(name)`·`Skill(name *)`로 allow/deny 가능.
...
| `description` | 권장 | 마크다운 첫 단락 | Claude가 언제 사용할지 판단 기준. **`description` + `when_to_use` 합산 1,536자 캡** (스킬 목록 표시 시). XML 태그 금지. |
| `when_to_use` | - | 없음 | **(2026-04 추가)** 트리거 구문·예시 요청 전용 필드. `description`과 분리해 유지보수 편의. 스킬 목록 표시 시 `description` 뒤에 이어붙여 1,536자 캡에 합산. |
| `argument-hint` | - | 없음 | 자동완성 시 보이는 힌트. 예: `[issue-number]`, `[filename] [format]` |
| `arguments` | - | 없음 | **(2026-04 추가)** 명명된 위치 인자 선언. 공백 구분 문자열 또는 YAML 리스트. 예: `arguments: [issue, branch]` → 본문에서 `$issue`, `$branch`로 참조. `$ARGUMENTS[0]` 대신 의미 있는 이름 사용 가능. |
| `paths` | - | 없음 | **(2026-04 추가)** 글로브 패턴 배열. 지정 시 Claude는 **매칭되는 파일 작업 시에만** 스킬을 자동 후보로 올림. 예: `["20-operations/{사업폴더}/**"]` → 특정 사업 폴더 작업 시만 자동 활성. 컨텍스트 절약. [path-specific rules](/en/memory#path-specific-rules)와 동일 문법. |
...
| `20-operations/{사업폴더}/**` | 특정 사업 폴더 전체 (사용자 예) |

`paths` 미지정 시 항상 자동 후보. `paths` 지정 시 해당 파일 작업할 때만 자동 활성 (수동 `/name` 호출은 언제나 가능).
...
- **기본값**: 일반적인 참고 자료, 코드 설명 스킬
- **`disable-model-invocation: true`**: `/commit`, `/deploy`, `/send-message` 등 부작용이 있거나 타이밍 제어가 필요한 작업
- **`user-invocable: false`**: Claude가 참고해야 하지만 사용자가 직접 실행할 필요 없는 백그라운드 지식
...
| 방법 | 위치 | 트리거 | 용도 |
|------|------|--------|------|
| **CLAUDE.md** | 루트 또는 폴더 내 | 항상 자동 로드 | 항상 적용되는 프로젝트 규칙/컨텍스트 |
| **Skills** | `.claude/skills/<name>/SKILL.md` | Claude 자동 감지 or `/name` | 자동 적용되는 복잡한 워크플로우 |
| **Commands** | `.claude/commands/<name>.md` | `/name` 직접 호출 | 수동으로 실행하는 명령 |
| **Subagents** | `.claude/agents/<name>.md` | Claude가 위임 | 독립된 컨텍스트에서 무거운 작업 |
| **Hooks** | settings.json / SKILL.md | 이벤트 기반 | 특정 도구 이벤트 시 스크립트 실행 |
| **MCP** | settings.json | Claude 필요시 호출 | 외부 도구/데이터 연결 |

> **핵심**: Commands와 Skills는 이제 통합됨. `.claude/commands/deploy.md`와 `.claude/skills/deploy/SKILL.md` 모두 `/deploy` 커맨드를 만들고 동일하게 작동. **Skills 사용 권장** (지원 파일, 자동 호출 등 추가 기능 제공).

### 내용 유형별 판단 기준

| 내용 | 어디에? |
|------|---------|
| 항상 적용되는 프로젝트 규칙 | `CLAUDE.md` |
| 수동으로 실행하는 반복 작업 | Skill + `disable-model-invocation: true` |
| Claude가 상황에 따라 자동 적용할 것 | Skill (기본값) |
| 무거운 리서치/탐색 작업 | Skill + `context: fork` |
| 사용자에게 숨길 백그라운드 지식 | Skill + `user-invocable: false` |
...
---
name: my-skill
description: [언제, 어떤 상황에서 이 스킬을 쓸지 구체적으로. 트리거 키워드 포함]

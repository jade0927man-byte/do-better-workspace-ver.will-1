...
## 🆕 2026-05-25 sync 반영 (공식 memory 문서 확인)

- `.claude/rules/*.md` — 토픽별 규칙 파일. 재귀 발견(하위 폴더 `frontend/` 등도). `paths` frontmatter 없으면 `.claude/CLAUDE.md`와 동일 우선순위로 매 세션 로드.
- **`paths` frontmatter (경로별 규칙)** — glob으로 지정 시 **매칭 파일 작업할 때만 로드** → 컨텍스트 절약:
  ```markdown
  ---
  paths:
    - "20-operations/{사업폴더}/**"
  ...
  ---
  ```
  표: `**/*.ts`(모든 ts) / `src/**/*`(src 하위 전부) / `*.md`(루트 md). brace expansion `{ts,tsx}` 지원.
- 사용자 레벨 `~/.claude/rules/` — 머신 전역. 프로젝트 룰이 더 높은 우선순위.
- 심볼릭 링크 지원(공유 룰 세트 링크), 순환 링크 자동 처리.
- **사용자 적용 포인트**: 전역 CLAUDE.md에 섞인 사업/투자/개인 룰을 `paths` 룰로 분리하면 "그 폴더 작업 때만" 로드 → 경량화. (단 적용 전 영향범위 확인 — feedback_rename_user_confirm)
- ⚠️ 상시 필요 없는 절차는 rules 말고 skill(호출 시만 로드)로.
